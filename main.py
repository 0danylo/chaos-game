...
def main():
    fractals = [
        ...
        [no_neighbors_if_last_n_same, 4, 2, 2], [no_neighbors_if_last_n_same, 4, 2, 2.6], 
        [no_neighbors_if_last_n_same, 4, 3, 2.6], [no_neighbors_if_last_n_same, 4, 2, 3.15],
        [no_neighbors_if_last_n_same, 5, 2, 2], [no_neighbors_if_last_n_same, 5, 2, 2.6], 
        [no_neighbors_if_last_n_same, 6, 1, 2], [no_neighbors_if_last_n_same, 6, 2, 2], 
        [no_cw_neighbor_if_last_n_same, 4, 2, 2], [no_cw_neighbor_if_last_n_same, 5, 10, 2], 
        [no_cw_neighbor_if_last_n_same, 6, 1, 1.5], [no_cw_neighbor_if_last_n_same, 6, 1, 1.8], 
        [no_cw_neighbor_if_last_n_same, 6, 1, 1.9], [no_cw_neighbor_if_last_n_same, 6, 1, 2], 
        [no_cw_neighbor_if_last_n_same, 6, 10, 1.85], [no_cw_neighbor_if_last_n_same, 8, 10, 1.7], 
        [restricted_fractal_2, 3, 2], [restricted_fractal_2, 3, 2.8], [restricted_fractal_2, 4, 2.25], 
    ...
    for i, fractal in enumerate(fractals):
        ...
        print(str(round((float(time.time() * 1000) - initial) / 1000, 3)) + 's')
        pygame.image.save(window, Path(f'images/fractal{i}.png'))
...
def get_reg_poly_vertices(n):
    ...
    if n == 3:
        ...
        vertices = ((bd, bd), (bd + size, bd),
                    (bd + size, bd + size), (bd, bd + size))
    else:
        for i in range(n):
...
def draw_pt(pt, n=0):
    """
        Add the amount of each color channel to the point's brightness, depending on n, which corresponds to the index of the color in clrs. Supports 6 colors and white.
    """
    x = int(pt[0])
    y = int(pt[1])
...
def normalize_brightness(new_max=255):
    """
        Normalizes the brightness of the fractal to the range [0, 255]. Using 256 shades with this function makes the fractal perfectly accurate, but quite dim since many points are only visited a few times. With more points, the fractal is more visually appealing, though only accurate up to a certain point.
    """
    max_br = 0
    ...
    """
        When the maximum brightness is low, the fractal is more visible because the brightness is more evenly distributed. This allows for a greater number of points, but its not necessary. However, this does not account for the black background on fairly empty fractals. Instead, the visibility should be calculated using the number of points visited. This can be used to determine the maximum number of points that can be placed without diminishing accuracy.
    """
    print(max_br)
