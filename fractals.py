...
def basic_fractal(verts, zoom):
    ...
    for i in range(1, iterations + 1):
        ...
        pt = (int(pt[0] + (verts[j][0] - pt[0]) / zoom),
              int(pt[1] + (verts[j][1] - pt[1]) / zoom))
        draw_at(pt)
...
def fractal_avoiding_center_circle(r, zoom):
    ...
    for i in range(1, iterations + 1):
        ...
        j = int(random() * 4)
        pt_q[0] = pt_q[1]
        pt_q[1] = (int(pt_q[1][0] + (verts[j][0] - pt_q[1][0]) / zoom),
                   int(pt_q[1][1] + (verts[j][1] - pt_q[1][1]) / zoom))
...
def restricted_fractal_2(zoom):
    ...
    verts = ((bd, bd), (bd + size, bd), (bd + size, bd + size), (bd, bd + size))
    ...
    for i in range(1, iterations + 1):
        ...
        if vt_q[0] == vt_q[1] or vt_q[0] == verts[(j + 1) % 4]:
            i -= 1
            continue
        pt = (int(pt[0] + (vt_q[1][0] - pt[0]) / zoom),
              int(pt[1] + (vt_q[1][1] - pt[1]) / zoom))
        draw_at(pt)
